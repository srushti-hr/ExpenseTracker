...
reply="Sorry 😅 I can answer questions about spending, income, categories, accounts and transactions.";
...
return`You spent ₹${total.toLocaleString()} today across ${count} transaction${count===1?"":"s"}.`;
...
if(
biggest===null||
Math.abs(item.amount)>Math.abs(biggest.amount)
){
biggest=item;
}
...
return`💸 Your biggest expense was ${biggest.title} for ₹${Math.abs(biggest.amount).toLocaleString()}.`;
